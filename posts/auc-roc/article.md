...
**1. True positive rate ($\text{TPR}$).** Also called **recall** or **sensitivity** — the fraction of positives we catch:
$$\text{TPR}(t) = \frac{\text{TP}}{\text{TP} + \text{FN}} = \frac{1}{|P|} \sum_{x \in P} \mathbb{I}\{S(x) \geq t\}$$

**2. False positive rate ($\text{FPR}$).** The complement of **specificity** — the fraction of negatives we mislabel:
$$\text{FPR}(t) = \frac{\text{FP}}{\text{FP} + \text{TN}} = \frac{1}{|N|} \sum_{x \in N} \mathbb{I}\{S(x) \geq t\}$$

ROC analysis splits the dataset into two isolated buckets:

- **The positives ($P$).** TPR is calculated only within this bucket.
- **The negatives ($N$).** FPR is calculated only within this bucket.
Each rate is therefore blind to what happens in the other class.
...
**Rank quality versus calibration.**
...
**Testing monotonicity through pair direction.**
...
**The relation between tau and AUC.**
