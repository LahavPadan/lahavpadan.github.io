...
Calculating the projection coefficients $\langle x_i, x_j \rangle = x_i^T x_j$. Attributing the importance to each vector:
...
Then, we output the following:

$$
\hat{x}_i = (q \cdot k_1)v_1 + (q \cdot k_2)v_2 + \ldots + (q \cdot k_n)v_n
$$
...
This enables us to force the projection coefficients to sum to one (like a probability distribution) using:
