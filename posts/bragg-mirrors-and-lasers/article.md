...
*[§ 1.1](#sec-1-1) fixed the two stopband edges $\omega_\pm$, [§ 1.4](#sec-1-4) their separation $\Delta k = 2\kappa$ — both for an infinite grating.
...
$\kappa L$ is the only knob. Reading it dimensionally: $\kappa$ is an inverse length (the field's own decay length, from [§ 2.1](#sec-2-1)), $L$ is a length, so $\kappa L$ measures the grating's physical length in units of $1/\kappa$. Setting $R > 0.99$,
...
We take the sinusoidal grating of [§ 1](#sec-1)–[§ 2](#sec-2) and replace it with what a real deposition tool produces: a stack of $N$ alternating high-index and low-index layers, each pair a **period**, with the high-index layer of thickness $d_H$ and refractive index $n_H$ and the low-index layer of thickness $d_L$ and refractive index $n_L$.
...
Expand this square-wave modulation as a Fourier series; only the fundamental cosine drives the first Bragg reflection, and its coefficient is $(4/\pi)$ times the peak-to-average amplitude of the square wave.

The effective sinusoidal amplitude is therefore $(2/\pi)\Delta n$, and
...
Both $k_B$ and $\Lambda$ cancel: the reflectivity depends only on the layer count and the fractional index contrast. Reaching $R > 0.99$ needs $\kappa L > \operatorname{arctanh}(\sqrt{0.99}) \approx 3$, so

$$N \gtrsim 3\, \frac{n_\text{avg}}{\Delta n}.$$

Two examples:

- **Dielectric stack** (Ta$_2$O$_5$ over SiO$_2$, $n_H/n_L \approx 1.5$ so $\Delta n / n_\text{avg} \approx 0.4$): $N \gtrsim 8$; real designs use 10–15 periods.

- **Semiconductor stack** (GaAs/AlAs, $n_H/n_L \approx 1.15$ so $\Delta n / n_\text{avg} \approx 0.14$): $N \gtrsim 22$; real designs use 25–30 periods.

The stopband width falls out of the same $\kappa$: $\Delta k = 2\kappa_\text{DBR}$, or $\Delta k / k_B = (2/\pi)(\Delta n / n_\text{avg})$. The fractional index contrast sets both design axes at once.
...
Each cosine harmonic is itself a periodic modulation, so each drives its own Bragg reflection: the $m$-th harmonic scatters a wave at wavenumber $m k_B$ into the wave at $-m k_B$, satisfying \eqref{eq:bragg} at *its* own resonance $k = m k_B$ — vacuum wavelength $\lambda_B / m$.
...
Each end is a single dielectric interface, so $R = \vert r_{12} \vert^2$ from \eqref{eq:fresnel}.
...
The field–gain overlap [\eqref{eq:gamma-eff}](#eq:gamma-eff) is the only asymmetry left between the two candidates.
...
- **Insert a $\lambda/4$ optical gap at the grating center.**
  ...
  Inside each half-grating, the field decays as $e^{-\kappa|z - L/2|}$ (from \eqref{eq:hyperbola} at $\delta = 0$), so the mode's extent is of order $\Lambda/2$ (the gap) plus $1/\kappa$ (the two exponential tails) — much shorter than $L$ itself.
  ...
  **The construction is robust:**
  ...
  because the gap is a physical structure written at fabrication, and the mode frequency $\omega_B$ is set by the gap geometry rather than by which half-grating happens to have slightly higher $\gamma_\text{eff}$, layer-thickness errors and $n_\text{avg}$ drifts of the kind listed above do not flip which mode wins.
...
- **Give the modulation a gain (imaginary) component.**
  ...
  When $\gamma(z)$ is a periodic function that itself alternates between high-gain and low-gain (or gain and loss) halves synchronised with the index modulation, the [\eqref{eq:gamma-eff}](#eq:gamma-eff) overlap integral picks up opposite values for the two:
  ...
  **The cost is fabrication:**

  modulating $\gamma(z)$ across $z$ requires physically corrugating the [gain](#defn-gain) region — patterning the current injection or the well structure period-by-period — which is much harder than the smooth index grating of the pure-index case.

  § 4.3 works through what happens when both modulations coexist and how far they can be chosen independently.
...
Reading along that axis: a central **gain section** carries $\gamma$ (a semiconductor gain medium, same as the DFB's) but no grating; one or both ends of the device continue into a passive **DBR section** carrying $\kappa$ (the Bragg grating) but no gain.
...
The design principle is \eqref{eq:bragg} wearing a different physical hat: a momentum kick from a periodic modulation, tuned to close a mismatch the underlying medium alone cannot close.
