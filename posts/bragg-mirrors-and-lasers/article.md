...
- The grating sits in a medium whose refractive index varies periodically along $z$: $n(z) = n_\text{avg} + \Delta n \cos(2 k_B z)$.
  ...
  - **The small parameter of the problem.**
    ...
    Every coupling scale we compute is proportional to it (with a modulation-shape-dependent prefactor: $1/2$ for the sinusoidal case here, $1/\pi$ for the square-wave stack of [§ 3.1](#sec-3-1)).
...
- Near this reference wavenumber, the field decomposes into forward and backward envelopes at $\pm k_B$,
  ...
  Non-sinusoidal modulations carry additional Fourier harmonics of $n(z)$, each of which drives its own pair at higher-order Bragg resonances $k = m k_B$ ([§ 3.2](#sec-3-2)).
...
[§ 1](#sec-1) and [§ 2](#sec-2) apply [\eqref{eq:hyperbola}](#eq:hyperbola), $q^2 = \delta^2 - \kappa^2$ at four values of $(\delta, q)$:
...
Inside the stopband ($|\delta| < \kappa$) [\eqref{eq:hyperbola}](#eq:hyperbola), $q^2 = \delta^2 - \kappa^2$ gives $q^2 < 0$: the field decays exponentially and no wave propagates.

The stopband is bounded above and below in $\omega$ by $\omega_-$ (lower) and $\omega_+$ (upper).

We find these two frequencies twice, from complementary starting points: the coupled-mode framework ([§ 1.1](#sec-1-1), [§ 1.2](#sec-1-2)) and the multilayer stack ([§ 1.3](#sec-1-3)).
...
Apply [\eqref{eq:rayleigh}](#eq:rayleigh) to the two edge modes. Both are pure sinusoids at the same wavenumber $k_B$, so their derivatives $dE/dz$ have the same shape and the numerators integrate to the same value.
...
Instead of the two-Fourier-component truncation of [§ 1.1](#sec-1-1), take the grating as a stack of layers of piecewise-constant index — the multilayer we will build as a [DBR](#defn-dbr) in [§ 3](#sec-3) — and follow the phase of the individual interface reflections.
...
- **Magnitude.**
  ...
  $\vert r_{12} \vert$ is small. Substituting $n_2 / n_1 = 1.5$ (glass to air) into [\eqref{eq:fresnel}](#eq:fresnel) gives $\vert r_{12} \vert = 0.2$; internal dielectric interfaces have less.
...
The rate at which they lose coherence is what sets the stopband width. [§ 1.4](#sec-1-4) works it out.

### § 1.4. The stopband width, from phase-error accumulation {#sec-1-4}

At $\omega = \omega_B$ (i.e. $\delta = 0$), each layer's round-trip phase is **exactly $\pi$**, causing reflections from all interfaces to combine constructively.
...
using $n_i d_i = \lambda_B/4 = \pi c/(2\omega_B)$ from [§ 1.3](#sec-1-3).

Each layer contributes a **phase error** of $0.01\,\pi \approx 2°$. Small at one layer, but the errors accumulate as the wave works deeper:
...
For this 1% [detuning](#defn-detuning), the **useful reflecting depth is $\sim 100$ layers** ($\approx 30\,\mu\text{m}$ of physical depth).

<div class="guided-fold-end"></div>

The phase argument shows that away from $\delta = 0$, deep and shallow reflections drift out of phase and eventually cancel each other. [\eqref{eq:hyperbola}](#eq:hyperbola), $q^2 = \delta^2 - \kappa^2$ makes this precise:
...
The **stopband** is therefore the wavenumber range $\vert\delta\vert < \kappa$ — a width $\Delta k = 2\kappa$ centered on $k_B$. Converting via the group velocity:
- **Frequency width:** $\Delta\omega = 2\kappa v_g$
- **Fractional stopband width:** Since $\kappa/k_B = \Delta n / (2 n_\text{avg})$ from the coupling definition, **half the fractional index contrast** fixes the fractional stopband width.

---

*[§ 1.1](#sec-1-1) fixed the two stopband edges $\omega_\pm$, [§ 1.4](#sec-1-4) their separation $\Delta k = 2\kappa$ — both for an infinite grating. A real grating has finite length, so whatever enters at $z = 0$ has to eventually exit at $z = L$ or come back out. What sets how much of the incident amplitude actually comes back?*
...
At $\delta = 0$, [\eqref{eq:hyperbola}](#eq:hyperbola), $q^2 = \delta^2 - \kappa^2$ gives $q^2 = -\kappa^2$ — the deepest negative value it attains inside the stopband:

- **At $\delta = 0$:** The decay length is $1/\kappa$ (the shortest one the grating produces) — the intrinsic length scale set by the modulation itself.
...
At $\delta = 0$ then, $q = \pm i\kappa$ and the two spatial factors are $e^{+\kappa z}$ and $e^{-\kappa z}$.

- **Infinite half-space ($z > 0$):** The $e^{+\kappa z}$ branch blows up at infinity and is dropped. The field decays as $e^{-\kappa z}$ — amplitude falls by $e$ over $1/\kappa$. This decay length is a property of the *medium*.
- **Finite cutoff ($z = L$):** Both branches must be kept. The second boundary is what selects the specific combination.

---

### § 2.2. Finite grating: the reflectivity {#sec-2-2}

Work at $\delta = 0$, per [§ 2.1](#sec-2-1): the algebra collapses to a single rate $\kappa$, and everywhere else inside the stopband follows by replacing $\kappa$ with $\alpha = \sqrt{\kappa^2 - \delta^2}$ throughout.
...
Differentiate either component of [\eqref{eq:cme}](#eq:cme) and substitute the other:
...
The reflected amplitude $B(0)$ and transmitted amplitude $A(L)$ fall out of [\eqref{eq:propag}](#eq:propag) by applying the **incident-wave boundary conditions**:
- $A(0) = 1$ (input rescaled to unit amplitude)
- $B(L) = 0$ (nothing propagating leftward from beyond $z = L$)

<div class="guided-fold-start" data-label="Solve the two-boundary problem for R and T" data-tone="derivation"></div>

- **Row 2 at the far end:** $0 = -i\sinh(\kappa L)\cdot 1 + \cosh(\kappa L)\, B(0)$, so $B(0) = i\tanh(\kappa L)$ and $R = \vert B(0) \vert^2 = \tanh^2(\kappa L)$.
- **Row 1 at the far end:** $A(L) = \cosh(\kappa L) + i\sinh(\kappa L)\cdot i\tanh(\kappa L) = \operatorname{sech}(\kappa L)$, so $T = \vert A(L) \vert^2 = \operatorname{sech}^2(\kappa L)$.
- **Energy conservation:** $R + T = 1$ follows from the identity $\tanh^2 + \operatorname{sech}^2 = 1$. It could have been anticipated: [\eqref{eq:cme}](#eq:cme)'s matrix is Hermitian, so $\vert A \vert^2 - \vert B \vert^2$ is constant along $z$ (differentiate; the cross terms cancel), and the boundary conditions turn that into $\vert A(L) \vert^2 = 1 - \vert B(0) \vert^2$.

$$\frac{d}{dz} \begin{pmatrix} A \\ B \end{pmatrix} = \begin{pmatrix} 0 & i\kappa \\ -i\kappa & 0 \end{pmatrix} \begin{pmatrix} A \\ B \end{pmatrix}.$$
...
**Comparing Semi-Infinite vs. Finite:**

- **Semi-infinite:** Field decays purely as $e^{-\kappa z}$ (one exponential mode, forced by boundedness at $z = \infty$).
- **Finite:** Both $\cosh$ and $\sinh$ branches are allowed; the far-end condition $B(L) = 0$ picks the specific combination [\eqref{eq:envelopes}](#eq:envelopes). As $\kappa L \to \infty$, the far end retreats to infinity and [\eqref{eq:envelopes}](#eq:envelopes) reduces to $A(z) \to A(0)\, e^{-\kappa z}$ near the entrance — the semi-infinite decay recovers as a limit.
...
**Physical Length Requirements:**

$\kappa L$ is the primary non-dimensional parameter:
- $\kappa$ is an inverse length (the field's own decay length, from [§ 2.1](#sec-2-1)).
- $L$ is physical length.
- $\kappa L$ measures the grating's physical length in units of $1/\kappa$.
...
*[§ 2.2](#sec-2-2) showed $R = \tanh^2(\kappa L)$, meaning $\kappa$ and $L$ jointly set the mirror. What sets $\kappa$ in a stack we actually build?*

## § 3. The DBR: a Bragg grating in fabricated hardware {#sec-3}

We take the sinusoidal grating of [§ 1](#sec-1)–[§ 2](#sec-2) and replace it with what a real deposition tool produces: **a stack of $N$ alternating high-index and low-index layers**:

- **Period:** each high-index/low-index pair
- **High-index layer:** thickness $d_H$, refractive index $n_H$
- **Low-index layer:** thickness $d_L$, refractive index $n_L$
- **Bragg period:** $\Lambda = d_H + d_L$
- **Total grating length:** $L = N\Lambda$

The physics is the same. What changes is that the modulation is now a **square wave** rather than a cosine, and three concrete questions follow:

- **What is $\kappa$ for a square-wave grating?**

  [§ 3.1](#sec-3-1) Fourier-expands the square wave; the fundamental cosine harmonic — at wavenumber $2 k_B$ — drives the Bragg reflection, and it sets $\kappa$ as a fraction of $k_B$ through the index contrast $\Delta n = n_H - n_L$.

  Combined with [§ 2.2](#sec-2-2)'s $R = \tanh^2(\kappa L)$, this fixes how many periods a mirror needs.

- **What do the higher Fourier components do?**

  [§ 3.2](#sec-3-2) shows they open additional stopbands at every odd multiple $m k_B$, each with a coupling $\kappa/m$ and correspondingly narrower, and lays out how to suppress or engineer them.

- **What happens off-normal?**

  [§ 3.3](#sec-3-3) adds the angle of incidence and shows how off-normal illumination reshapes the geometry and splits the polarizations.
...
A [DBR](#defn-dbr) is instead *piecewise-constant* — the refractive index takes only two values, $n_H$ and $n_L$.

#### Fourier expansion and coupling coefficient

Expanding this square-wave modulation as a Fourier series reveals:

1. Only the **fundamental cosine** drives the first Bragg reflection.
2. Its coefficient is **$(4/\pi)$** times the peak-to-average amplitude of the square wave.
3. The effective sinusoidal amplitude is therefore **$(2/\pi)\Delta n$**.

This gives the DBR coupling coefficient:
...
Combine with [§ 2.2](#sec-2-2)'s $R = \tanh^2(\kappa L)$. The total grating length is $L = N\Lambda$, and $k_B \Lambda = \pi$ from $k_B = \pi/\Lambda$, so

$$\kappa_\text{DBR}\, L = N\, \frac{\Delta n}{n_\text{avg}}.$$

> **Key takeaway:** Both $k_B$ and $\Lambda$ cancel — the reflectivity depends **only on the layer count and the fractional index contrast**.

Reaching $R > 0.99$ needs $\kappa L > \operatorname{arctanh}(\sqrt{0.99}) \approx 3$, so

$$N \gtrsim 3\, \frac{n_\text{avg}}{\Delta n}.$$

#### Material-system examples

Two examples:

- **Dielectric stack** (Ta$_2$O$_5$ over SiO$_2$):
  - $n_H/n_L \approx 1.5$, so $\Delta n / n_\text{avg} \approx 0.4$.
  - $N \gtrsim 8$; real designs use 10–15 periods.

- **Semiconductor stack** (GaAs/AlAs):
  - $n_H/n_L \approx 1.15$, so $\Delta n / n_\text{avg} \approx 0.14$.
  - $N \gtrsim 22$; real designs use 25–30 periods.
...
The fractional index contrast sets both design axes at once.
...
Each cosine harmonic is itself a periodic modulation, so each drives its own Bragg reflection:

- **Resonance:** the $m$-th harmonic scatters a wave at wavenumber $m k_B$ into the wave at $-m k_B$, satisfying [\eqref{eq:bragg}](#eq:bragg), $\lambda_B = 2\, n_\text{avg}\, \Lambda$ at *its* own resonance $k = m k_B$ — vacuum wavelength $\lambda_B / m$.
- **Coupling:** the coupling for the $m$-th stopband is $\kappa_m = \kappa_\text{DBR}/m$ (the $1/m$ Fourier coefficient).
- **Stopband width:** the stopband width scales the same way — the third-order stopband is $1/3$ as wide as the first, the fifth $1/5$, and so on.
- **Even orders:** even orders are absent because the square wave has no even Fourier components.
...
#### 1. The one-way phase per layer picks up a factor of $\cos\theta_i$
...
From [§ 1.3](#sec-1-3), the Bragg condition requires the sum of one-way phases across one H+L period to equal $\pi$:
...
- To hold the sum at $\pi$, the wavenumbers $k_H, k_L$ must grow.
- Since $k_i = n_i \omega/c$ with $n_i$ fixed, the operating frequency $\omega$ grows with them.

> **Result:** Writing the operating in-medium Bragg wavenumber as $k_B(\theta_0) = n_\text{avg}\, \omega/c$, this reads $k_B(\theta_0) > k_B(0)$: the Bragg wavenumber shifts *up* with incidence angle, and the vacuum wavelength shifts *down*.

#### 2. TE and TM see different boundary conditions at each interface

- **TE polarization:** $\vec{E}$ is perpendicular to the plane of incidence, entirely tangent to every layer face — the two matching conditions (tangential $E$ and tangential $H$ continuous) both act on components already parallel to the interface.
- **TM polarization:** $\vec{E}$ lies in the plane of incidence, with a *normal* component; that normal component is discontinuous across the interface because it is $D_\perp = \varepsilon\, E_\perp$ (not $E_\perp$) that must be continuous.

TE and TM therefore solve different linear systems at each interface, and [\eqref{eq:fresnel-full}](#eq:fresnel-full) below gives their reflection coefficients.

#### 3. The two stopbands split ($\kappa_\text{TE} \neq \kappa_\text{TM}$)

Since $\kappa$ is built from the per-interface reflections in [§ 1.3](#sec-1-3), $\kappa_\text{TE} \neq \kappa_\text{TM}$ off-normal — TE and TM acquire distinct stopband widths $2\kappa_\text{TE} v_g$ and $2\kappa_\text{TM} v_g$ at the same Bragg wavelength.
...
At $\theta_1 = 0$ both reduce to $r_{12} = (n_1 - n_2)/(n_1 + n_2)$ — [\eqref{eq:fresnel}](#eq:fresnel). Off-normal, they split. Setting $r_\text{TM} = 0$ requires the numerator of [\eqref{eq:fresnel-full}](#eq:fresnel-full) to vanish,
...
The geometric reading: $r_\text{TM} = 0$ exactly when the reflected and transmitted rays are perpendicular ($\theta_1 + \theta_2 = \pi/2$); [\eqref{eq:brewster}](#eq:brewster) is that perpendicularity re-expressed through Snell's law.

{% include visualization.html src="brewster.html" title="How the TE and TM Fresnel responses separate at Brewster incidence" %}

#### Physical picture of Brewster's angle

1. The transmitted electric field polarizes the electrons of medium 2 into oscillating dipoles along its own direction, and the reflected wave is their re-radiation.
2. A dipole cannot radiate along its own axis of oscillation — the null of its $\sin\theta$ pattern.
3. At Brewster's angle Snell's law makes the reflected direction (in medium 1) coincide with the dipole axis (which points along the transmitted $E$ in medium 2), so the reflection has to vanish.
4. TE has no analogue: its dipoles point out of the plane of incidence, and every in-plane direction — including every possible reflected direction — is broadside to them.
...
#### DBR consequences at Brewster incidence

The consequence for a [DBR](#defn-dbr): as the in-medium angle approaches Brewster's angle,

- the TM interface reflection coefficient $r_\text{TM}$ shrinks toward zero ([\eqref{eq:brewster}](#eq:brewster), $\tan\theta_1 = n_2/n_1$),

- the TM [coupling](#defn-coupling) $\kappa_\text{TM}$ is proportional to $r_\text{TM}$ (from the sum-of-per-interface-reflections argument in [§ 1.3](#sec-1-3)), so $\kappa_\text{TM} \to 0$ with $r_\text{TM}$,
...
A laser combines two ingredients:

- A **gain medium**{:#defn-gain}, which amplifies the field.
- A **cavity**{:#defn-cavity}, which feeds the field back on itself so that [gain](#defn-gain) accumulates coherently over many round trips.
...
- **[DFB](#defn-dfb) laser:** writes the grating into the same section as the [gain](#defn-gain) — [coupling](#defn-coupling) $\kappa$ and [gain](#defn-gain) $\gamma$ act at every $z$.
- **DBR laser:** puts the grating in a separate section from the [gain](#defn-gain) — $\gamma$ acts only where the [gain](#defn-gain) sits, $\kappa$ only where the grating sits, and the two sections can be tuned independently.
...
Take the simplest cavity: a gain-carrying dielectric of length $L$ and refractive index $n_\text{avg}$ (same symbol as the rest of the post — the modulation of [§ 1](#sec-1) simply hasn't been switched on yet), bounded by two flat interfaces at which the index steps back to air.

Two features of this setup decide what the laser does.

#### 1. A comb of resonant modes
...
#### 2. A mirror with no wavelength selectivity

Each end is a single dielectric interface, so $R = \vert r_{12} \vert^2$ from [\eqref{eq:fresnel}](#eq:fresnel), $r_{12} = (n_1 - n_2)/(n_1 + n_2)$.
...
- **Broad bandwidth:** its bandwidth is of order THz — thousands of $\Delta\nu_\text{FSR}$ fit inside it, so many modes see comparable gain at any instant.

- **Thermal and current drift:** its peak position is not fixed: drive current and chip temperature both shift the peak during operation, and as the peak moves the winning mode hops with it.
...
Near a stopband boundary $\omega_\pm$:

- A wave propagates with group velocity $v_g = d\omega/dq \to 0$ ([§ 0.7 of the previous post](/posts/coupled-modes-bragg-structures-and-photonic-bandgaps/#sec-0-7)) — the slope of the dispersion curve vanishes at the stopband boundary.
- Slow light accumulates the same round-trip phase over one or two decay lengths $1/\kappa$ that a fast wave would need the full flat-mirror $2L$ for.

Two twists follow from this:
...
- **Inside the stopband ($|\delta| < \kappa$):**
  - By [\eqref{eq:hyperbola}](#eq:hyperbola), $q^2 = \delta^2 - \kappa^2$, $q^2 < 0$; the envelope decays as $e^{-\alpha z}$ with $\alpha = \sqrt{\kappa^2 - \delta^2}$ ([previous post § 0.6](/posts/coupled-modes-bragg-structures-and-photonic-bandgaps/#sec-0-6)).
  - Over a grating of length $L$ with $\kappa L \gtrsim 2$, the envelope has decayed to a small fraction of its input at the far end. Flux conservation $\vert A \vert^2 - \vert B \vert^2 = T$ from [§ 2.2](#sec-2-2) then gives $R \to 1$.
...
The lasing candidates are reduced from the thousands of the flat-mirror comb ([§ 4.1](#sec-4-1)) to the modes fitting inside the width $2\kappa v_g$ ([§ 1.4](#sec-1-4)) — a comb of about $4\kappa L/\pi$ modes, so of order 10 for a typical $\kappa L \approx 3$ device.
...
Two effects push the emitters toward the two $\omega_\pm$ standing modes of [§ 1](#sec-1) — both at $\delta = \pm\kappa$, the two stopband edges. Together they decide which of the discrete lasing candidates gets to lase.

**1. More available modes per frequency at $\omega_\pm$**
...
- **Why $\rho \to \infty$ as $\omega \to \omega_\pm$.**
  ...
  At the stopband edge $q = 0$ (from [§ 1.1](#sec-1-1)), and the group velocity $v_g$ vanishes: the two branches of the dispersion meet with a horizontal tangent, $d\omega/dq = 0$. Expand $\omega(q)$ around $q = 0$:
...
There are of order 10 lasing candidates fitting inside the stopband ([§ 4.2](#sec-4-2)).
...
**2. Field–gain spatial overlap**

The $\omega_-$ and $\omega_+$ standing waves peak in complementary halves of one [Bragg period](#defn-bragg-period) ([§ 1.1](#sec-1-1)) — $\omega_-$ (the cosine mode) in the high-index material, $\omega_+$ (the sine mode) in the low-index material.
...
The two effects of [§ 4.2.1](#sec-4-2-1) push in the same direction — both favour $\omega_-$ over $\omega_+$ — but only by a small margin.

Both stopband edges are equally sharp, so [§ 4.2.1](#sec-4-2-1) alone does not distinguish between them: the same $\rho \to \infty$ at both.

The field–gain overlap from [\eqref{eq:gamma-eff}](#eq:gamma-eff), $\gamma_\text{eff} = \frac{\int \gamma(z)|E(z)|^2\,dz}{\int |E(z)|^2\,dz}$, is the only asymmetry left between the two candidates.
...
- **Layer thickness errors.**
  ...
  The overlap integral in [\eqref{eq:gamma-eff}](#eq:gamma-eff), $\gamma_\text{eff} = \frac{\int \gamma(z)|E(z)|^2\,dz}{\int |E(z)|^2\,dz}$, then favours a different mode than the design intended.
...
- **Insert a $\lambda/4$ optical gap at the grating center:**
  - Cut the grating in half at $z = L/2$ and insert a short unmodulated section of optical length $\lambda_B/4$ (physical length $\lambda_B / (4 n_\text{avg})$, equal to $\Lambda/2$).
  - Two halves of Bragg grating on either side of an empty gap in the middle.
  - This turns the DFB into a small Fabry–Perot cavity: the gap is the cavity, the two half-gratings are its mirrors.
  - Both mirrors reflect only inside the [stopband](#defn-stopband) ([§ 4.2](#sec-4-2)), so the cavity's resonances are constrained to $|\delta| < \kappa$.
  - Inside that range, the resonance condition — round-trip phase equal to $2\pi n$ — picks out a specific frequency.
  - For a gap of optical length $\lambda_B/4$ (round-trip optical length $\lambda_B/2$, i.e. round-trip propagation phase $\pi$), combining the propagation phase with the reflection phases of the two Bragg-mirror halves selects $\delta = 0$: the center of the stopband.
  - A single mode.
  - Spatially, this mode is localized around the gap.
  - Inside each half-grating, the field decays as $e^{-\kappa|z - L/2|}$ (from [\eqref{eq:hyperbola}](#eq:hyperbola), $q^2 = \delta^2 - \kappa^2$ at $\delta = 0$), so the mode's extent is of order $\Lambda/2$ (the gap) plus $1/\kappa$ (the two exponential tails) — much shorter than $L$ itself.
  - Its frequency is $\omega_B$ (the exact center between $\omega_-$ and $\omega_+$), not either of the edges, so it has no partner at the same frequency to compete with.
  - **The construction is robust:** because the gap is a physical structure written at fabrication, and the mode frequency $\omega_B$ is set by the gap geometry rather than by which half-grating happens to have slightly higher $\gamma_\text{eff}$, layer-thickness errors and $n_\text{avg}$ drifts of the kind listed above do not flip which mode wins.

- **Give the modulation a gain (imaginary) component:**
  - So far the modulation has been purely in the real part of $\varepsilon(z)$.
  - Extend it: let $\varepsilon(z)$ carry a gain modulation too, so that the [gain](#defn-gain) rate $\gamma(z)$ is itself a periodic function of $z$ with the same period as the index grating.
  - Recall from [§ 4.2.1](#sec-4-2-1) that $\omega_-$ peaks in the high-index material and $\omega_+$ in the low-index material — the two standing waves have their intensity concentrated in *complementary* halves of one Bragg period.
  - When $\gamma(z)$ is a periodic function that itself alternates between high-gain and low-gain (or gain and loss) halves synchronised with the index modulation, the overlap integral from [\eqref{eq:gamma-eff}](#eq:gamma-eff), $\gamma_\text{eff} = \frac{\int \gamma(z)|E(z)|^2\,dz}{\int |E(z)|^2\,dz}$, picks up opposite values for the two:
  ...
  - The mode with the larger effective gain wins by a margin set by the gain modulation depth, not by the small overlap asymmetry that the pure-index case relied on. No structural shift needed.
  - **The cost is fabrication:** modulating $\gamma(z)$ across $z$ requires physically corrugating the [gain](#defn-gain) region — patterning the current injection or the well structure period-by-period — which is much harder than the smooth index grating of the pure-index case.
  - [§ 4.3](#sec-4-3) works through what happens when both modulations coexist and how far they can be chosen independently.
...
- **$\Delta\varepsilon'$ (real):** modulates the refractive index.
- **$\Delta\varepsilon''$ (imaginary):** modulates [gain](#defn-gain) or loss (see [the complex-response section of the previous post](/posts/coupled-modes-bragg-structures-and-photonic-bandgaps/#sec-0-8) for the sign convention).

**Comparison of limits:**

- **$\Delta\varepsilon'' = 0$ (pure index modulation):**
  - $\omega_-$ and $\omega_+$ sit at the two [stopband](#defn-stopband) edges with nearly the same $\gamma_\text{eff}$. Mode selection is fragile; the $\lambda/4$ shift of [§ 4.2.2](#sec-4-2-2) is what forces a single mode.
...
- **$\Delta\varepsilon' = 0$ (pure gain modulation):**
  ...
  - Evaluating the overlap in [\eqref{eq:gamma-eff}](#eq:gamma-eff), $\gamma_\text{eff} = \frac{\int \gamma(z)|E(z)|^2\,dz}{\int |E(z)|^2\,dz}$, for each:
...
In reality, the emission spectrum is not a delta function:

- It has a **finite full width at half maximum (FWHM)** $\Delta\nu$.
- The physics that sets that width is quite different from what fixes the center frequency.
...
- **$A(t)$:** slowly-varying amplitude
- **$\phi(t)$:** slowly-varying phase

If $\phi(t)$ were exactly constant, the Fourier spectrum of $E(t)$ would be a delta function at $\omega_B$: an infinitely narrow line.
...
- **Full width at half maximum:** $\Delta\omega = 1/\tau_\text{coh}$
- **Linear-frequency linewidth:**

  $$\Delta\nu = \frac{1}{2\pi\, \tau_\text{coh}}.$$

<div class="guided-fold-end"></div>

So the question "what is $\Delta\nu$?" is the question "how fast does the phase $\phi(t)$ drift?" The rest of [§ 4.4](#sec-4-4) traces where the drift comes from and how big it is.
...
- **Leakage through the two grating ends:**
  - At each end, the fraction of the intracavity power that transmits out per pass is $T = 1 - R = 1 - \tanh^2(\kappa L) = \text{sech}^2(\kappa L)$ ([§ 2.2](#sec-2-2)).
...
- **Stimulated emission:**
  - A photon already in a specific mode induces the excited carrier to drop.
  - The emitted photon is a copy of the trigger — same mode, same frequency, same phase, adding coherently to the field already there.
...
- **Spontaneous emission:**
  - The carrier drops without any triggering photon.
  - The emitted photon goes into some mode chosen at random from those coupled to the transition, and its phase is uncorrelated with anything in the mode already.
  - The rate is set by the atomic transition itself and by the density of available modes ([§ 4.2.1](#sec-4-2-1)), not by how many photons are currently there.
...
The intracavity photon number is related to the output power. Photons leak out at rate $1/\tau_\text{leak}$ (from [§ 4.4.2](#sec-4-4-2), ignoring absorption for the output-power accounting), each carrying energy $h\nu$, so
...
- **$\Delta\nu \propto 1/P_\text{out}$:** more output power means a larger $N_\text{ph}$ swamping each unit-magnitude kick. Explicit in the phase-kick geometry of [§ 4.4.4](#sec-4-4-4).
...
The Schawlow–Townes formula counts only the *direct* phase kick of [§ 4.4.4](#sec-4-4-4) — the geometric perturbation of the phasor by an added photon.
...
2. By the causal-response relations of [§ 4.3.1](#sec-4-3-1), $\varepsilon'(z)$ (the refractive index) and $\varepsilon''(z)$ (the gain) *cannot vary independently* — a carrier-density change that shifts one shifts the other.

   Concretely, in the semiconductor: increasing $N$ raises $\gamma$ (more excited carriers to emit) and simultaneously lowers $n_\text{avg}$ (Kramers–Kronig companion, [§ 4.5](#sec-4-5)'s bandfilling and free-carrier discussion).
...
The period $\Lambda$ is set once by the geometric pattern written into the chip at fabrication, so tuning after the fact must act on $n_\text{avg}$ or on $\Lambda$ indirectly (through overall dimensional change of the substrate).

Two mechanisms are practical:

- **Temperature:**
  - Heating the chip shifts both factors — $n_\text{avg}$ rises slightly with temperature (an empirical coefficient of the material, $dn/dT$ of order $10^{-4}$/K in a semiconductor), and the substrate expands, stretching $\Lambda$.
  - Both terms push $\lambda_B$ in the same direction, giving of order $0.1$ nm of Bragg-wavelength shift per K. Range around 5 nm; the response is limited by how long the heat takes to spread through the chip volume from its point of application to the DFB region — a heat-diffusion time of order milliseconds for a chip a few hundred µm thick.

- **Current injection:**
  - Passing extra current through the [DFB](#defn-dfb) injects extra free carriers into the [gain](#defn-gain) medium. Two mechanisms then shift $n_\text{avg}$:
    1. **Free-carrier polarizability.**
    ...
    2. **Bandfilling.**
       ...
       - That absorption drop — a change in $\varepsilon''$ over a range of frequencies — is coupled by the Kramers–Kronig relations of [§ 4.3.1](#sec-4-3-1) to a change in $\varepsilon'$ across the *same* range, lowering $n_\text{avg}$ at the laser's operating frequency.
  - Both mechanisms push $n_\text{avg}$ down and are fast: the injected carriers reach a new steady state on the carrier lifetime, of order nanoseconds.
  - The tuning coefficient is small — about $0.01$ nm/mA — and this knob is not independent of the drive point: the same current that shifts $n_\text{avg}$ also sets the laser's output power (more current → more carriers → more stimulated emission), so retuning changes the emitted power as a side effect.
...
The Bragg-wavelength tuning range is capped at a few nm — that is the operational reach a DFB gives us on its emission frequency.

Wider tuning requires making $\kappa$ and $\gamma$ act in physically distinct sections of the device, so their indices can move independently.
...
Reading along that axis:

- A central **gain section** carries $\gamma$ (a semiconductor gain medium, same as the DFB's) but no grating.
- One or both ends continue into a passive **DBR section** carrying $\kappa$ (the Bragg grating) but no gain.

The gain section and each DBR section have separate metal contacts on top,

so the current injected into the gain section (setting $\gamma$, and with it the output power) and the current injected into the DBR section (used to shift the DBR-section $n_\text{avg}$ through the carrier-injection mechanism of [§ 4.5](#sec-4-5), and with it $\omega_B$) are controlled by two independent power supplies.

Three tuning strategies follow from that independence:

| Strategy | Mechanism | Result |
| --- | --- | --- |
| **Grating-only tune** | Change the index of the [DBR](#defn-dbr) section while leaving the [gain](#defn-gain) section fixed. | $\omega_B$ moves while the total round-trip length and mode spacing remain essentially unchanged; as $\omega_B$ crosses successive [cavity](#defn-cavity) modes, the laser hops from one to the next. |
| **Cavity-only tune** | Insert a passive phase-shift section between the grating and the [gain](#defn-gain), then change its index. | The [cavity](#defn-cavity) mode frequencies move without moving $\omega_B$; continuous tuning over one free spectral range is possible without a mode hop. |
| **Combined** | Tune both sections together. | The selected [cavity](#defn-cavity) mode stays centered inside the [stopband](#defn-stopband) as the stopband slides; this gives the widest continuous-tuning range with a single grating. |

The [DBR](#defn-dbr) laser has more sections to control than a [DFB](#defn-dfb) — typically three or four, each with its own contact — but the combined strategy reaches further, without mode hops.
...
[§ 1](#sec-1)–[§ 4](#sec-4) treated the grating as uniform: constant [coupling](#defn-coupling) $\kappa$ and constant period $\Lambda$.
...
Inside the [stopband](#defn-stopband) the reflection saturates near unity as $\kappa_0 L$ grows large ([§ 2.2](#sec-2-2)); outside the [stopband](#defn-stopband) reflection is weak but not zero — the spectrum carries a train of secondary maxima on both sides of $\lambda_B$.
...
Start from [\eqref{eq:r-of-delta}](#eq:r-of-delta):

$$r(\delta) \approx i \int_0^L \kappa(z)\, e^{2i\delta z}\, dz.$$
...
At depths where $\lambda \neq \lambda_B(z)$, the phase mismatch $2\delta = 2(k - k_B(z))$ is nonzero, and successive periods contribute to the reflection with rotating phases that cancel to leading order — the same phase-error accumulation of [§ 1.4](#sec-1-4), local to that depth.
...
A **chirped fiber Bragg grating** (CFBG) engineered with the opposite sign of $d\tau/d\lambda$ in [\eqref{eq:chirp-delay}](#eq:chirp-delay), $\tau(\lambda) = \frac{2 n_\text{avg}}{c\,\alpha\,\lambda_{B,0}}(\lambda-\lambda_{B,0})$ — choose $\alpha$ so that $d\tau/d\lambda = -D_\text{fiber} \cdot z_\text{fiber}$ — applies the reverse delay in reflection, and the emerging pulse is recompressed to its original width.
...
The "free propagation" approximation used to derive [\eqref{eq:chirp-delay}](#eq:chirp-delay), $\tau(\lambda) = \frac{2 n_\text{avg}}{c\,\alpha\,\lambda_{B,0}}(\lambda-\lambda_{B,0})$ ignores the last piece of the round trip: as the wave enters the local stopband near $z^*$ and turns around inside it, it slows down and picks up an extra contribution to $\tau(\lambda)$ from that region.
...
The mechanism is a direct reading of [\eqref{eq:hyperbola}](#eq:hyperbola), $q^2 = \delta^2 - \kappa^2$ at $\delta = \pm\kappa$: the group velocity $d\omega/dq$ vanishes at the two [stopband](#defn-stopband) boundaries $\omega_\pm$ of [§ 1](#sec-1).
...
These look almost like the counter-propagating equations [\eqref{eq:cme}](#eq:cme), but with a critical sign difference: both terms on the right-hand side have coefficients of the same sign, not opposite signs. That single sign change flips the conservation law of the system.

$$\frac{d}{dz} \begin{pmatrix} A \\ B \end{pmatrix} = \begin{pmatrix} 0 & i\kappa \\ -i\kappa & 0 \end{pmatrix} \begin{pmatrix} A \\ B \end{pmatrix}.$$

Computing $d(\vert A_1 \vert^2 + \vert A_2 \vert^2) / dz$ from [\eqref{eq:copropag}](#eq:copropag) (differentiate each modulus squared, use the equations to eliminate $d A_i / dz$),
...
The design principle is [\eqref{eq:bragg}](#eq:bragg), $\lambda_B = 2\, n_\text{avg}\, \Lambda$ wearing a different physical hat: a momentum kick from a periodic modulation, tuned to close a mismatch the underlying medium alone cannot close.

---

## Closing

[§ 1](#sec-1) and [§ 2](#sec-2) read [\eqref{eq:hyperbola}](#eq:hyperbola) — the Bragg dispersion $q^2 = \delta^2 - \kappa^2$ — at four operating points:
