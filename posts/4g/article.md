...
### 1.2. The two hard turns {#sec-1-2}
...
### 2.2. The RAN: the eNodeB alone {#sec-2-2}
...
### 3.2. Coherence bandwidth and its reciprocal relationship with delay spread {#sec-3-2}
...
### 3.3. Flat vs frequency-selective fading — the key design consequence {#sec-3-3}
...
For this to be feasible, the subcarrier spacing $\Delta f$ must satisfy
...
<div class="guided-fold-start" data-label="Derive why the cyclic prefix produces one scalar equation per subcarrier" data-tone="derivation"></div>
...
Write the linear convolution as

$$\mathbf{y} = H_{\text{lin}} \mathbf{x} + \text{(ISI from previous block)}.$$

Assuming for a moment there is no previous block (or ignoring the contamination at the start), $H_{\text{lin}}$ for $N=6, L=2$ is
...
Take the DFT of the received block:

$$Y = F \mathbf{y} = F H_{\text{circ}} \mathbf{x} + F \mathbf{n} = \Lambda F \mathbf{x} + F \mathbf{n} = \Lambda X + N.$$

Because $\Lambda$ is diagonal, this reads componentwise as

$$Y[k] = H[k] \cdot X[k] + N[k], \qquad k = 0, 1, \ldots, N-1.$$

Each subcarrier is one independent scalar equation. The equaliser is one complex division—the **single-tap equaliser**:
...
### 3.5. What CP length is required {#sec-3-5}
...
### 3.6. Symbol duration is the reciprocal of subcarrier spacing {#sec-3-6}
...
For $\Delta f = 15\,\text{kHz}$, $T_u = 66.67\,\mu\text{s}$. Total OFDM symbol length including CP is

$$T_{\text{sym}} = T_u + T_{\text{CP}} \approx 71.4\,\mu\text{s}$$

for normal CP.

### 3.7. The Doppler direction {#sec-3-7}
...
Rule of thumb: for tolerable ICI,

$$\Delta f \geq 10 f_d.$$

At $650\,\text{Hz}$ this demands

$$\Delta f \geq 6.5\,\text{kHz}.$$
...
### 4.1. Subcarrier spacing {#sec-4-1}
...
- UMTS used a chip rate of 3.84 Mcps, driven by a 15.36 MHz baseband sample rate. To reuse UMTS RF hardware in early LTE base stations, LTE's sample rate had to be a rational multiple of 15.36 MHz. The choice $\Delta f = 15\,\text{kHz}$ makes $15 \times 1024 = 15{,}360\,\text{kHz}$, matching UMTS at the 10 MHz LTE bandwidth. Similarly $15 \times 2048 = 30{,}720\,\text{kHz}$ for 20 MHz LTE — the 30.72 MHz sample rate is exactly $8 \times$ the UMTS chip rate.

$15\,\text{kHz}$ sits in the middle of the physics window *and* makes the FFT sample rate a clean multiple of the inherited UMTS clock.

### 4.2. Symbol duration and CP length {#sec-4-2}
...
### 4.3. 7 symbols per slot, 0.5 ms per slot {#sec-4-3}
...
Historical note: a "slot" was a first-class scheduling unit in UMTS at 0.667 ms; the LTE choice of 0.5 ms is a compromise that gives a rounder number while keeping the same order.

<div class="guided-fold-end"></div>

### 4.4. TTI = 1 ms — the silicon anchor {#sec-4-4}
...
### 4.6. 12 subcarriers per resource block {#sec-4-6}

A **resource block (RB)** is the smallest allocation unit the scheduler can hand out. One RB spans one slot in time (7 OFDM symbols, $0.5\,\text{ms}$) and $12$ consecutive subcarriers in frequency — at $\Delta f = 15\,\text{kHz}$, that is $180\,\text{kHz}$ wide.
...
### 4.8. The special first CP {#sec-4-8}
...
### 5.2. When each level matters — the rules {#sec-5-2}
...
### 5.4. When subcarriers matter vs when RBs matter — an example {#sec-5-4}
...
### 5.5. The full grid in one picture {#sec-5-5}
...
### 6.2. SC-FDMA as DFT-precoded OFDM {#sec-6-2}
...
### 6.3. Why PUCCH sits at the band edges {#sec-6-3}
...
### 6.4. The one-carrier promise, kept loosely {#sec-6-4}
...
Uplink DMRS solves the adjacent problem: there is no CRS on the uplink at all (the UE cannot afford to broadcast pilots across a band it isn't using), so the eNodeB has no channel estimate for the UE's PUSCH until the UE puts a pilot on the subcarriers it was granted.

Either way, DMRS lives only in the REs of a specific PDSCH or PUSCH grant. No grant, no DMRS. Unlike CRS, DMRS is per-grant and per-UE, and disappears when the allocation does.

### 7.3. Sounding Reference Signal (SRS) — uplink wideband probe {#sec-7-3}
...
### 8.3. Dimension splitting — why two length-31 sequences instead of one length-63 {#sec-8-3}

The SSS occupies 62 subcarriers. A natural first thought is: use one **m-sequence** of length 63.

<div class="guided-fold-start" data-label="Why m-sequences are suitable synchronization sequences" data-tone="derivation"></div>
...
<div class="pull-out" markdown="1">
**General LTE principle — dimension splitting.** When one parameter needs to carry multiple pieces of information, decompose it into a product space where each factor carries one piece. The 504 total PCIs are
...
### 8.4. Where PSS and SSS sit in the grid — FDD vs TDD {#sec-8-4}
...
Beyond SIB1 and SIB2, the standard defines a family of SIBs each answering one class of runtime question. Only the ones a UE actually needs in its current state are read; the others are ignored. A UE about to attach reads SIB1 and SIB2; an idle UE monitoring for handover reads SIB3–SIB5; warning notifications (SIB10–SIB11, SIB12) are read immediately whenever their flags in SIB1 are set — this is the mechanism for earthquake early warning and Amber Alerts on cellular.
...
### 9.5. Where SIBs live physically {#sec-9-5}
...
Time axis (eNodeB's clock)
   ...
   Downlink boundary sent at t0_eNB
        |
        +---- propagation tau ---->
                                  |
                                  Downlink arrives at UE at t0_UE = t0_eNB + tau
                                  |
    Without TA:                   |
                                  UE transmits at t0_UE
                                  --- propagation tau ---->
                                                          |
                                                          Uplink arrives at t0_eNB + 2*tau  <- MISALIGNED

    With TA = 2*tau:
                          UE transmits 2*tau earlier, at t0_UE - 2*tau
                          --- propagation tau ---->
                                                  |
                                                  Uplink arrives at t0_eNB  <- ALIGNED
~~~

### 10.1. Why timing advance is a real problem {#sec-10-1}
...
### 10.2. The Zadoff-Chu preamble and why it looks the way it does {#sec-10-2}
...
### 10.3. Cyclic shifts — extracting many preambles from one root {#sec-10-3}
...
<div class="pull-out" markdown="1">
**Identification rule.** A peak from one correlator, at some position $p$, has two candidate explanations:
...
### 10.4. Why the PRACH waveform is different from ordinary uplink {#sec-10-4}
...
PRACH's solution: use $\Delta f_{\text{PRACH}} = 1.25\,\text{kHz}$ — twelve times smaller than the $15\,\text{kHz}$ used everywhere else on the uplink, which stretches the useful symbol duration by the same factor:
...
### 10.6. Contention-based vs contention-free random access {#sec-10-6}
...
### 11.3. Aggregation levels — trading coding rate for CCE count {#sec-11-3}
...
### 12.1. What the UE knows going in {#sec-12-1}
...
### 12.2. Search spaces bound the work {#sec-12-2}
...
### 12.3. The candidate-position formula {#sec-12-3}

The eNodeB knows the UE's C-RNTI, so it computes the same pseudo-random candidate set the UE will search and places the DCI at one of those positions. Nothing needs to be signalled about which position was chosen; the CRC-with-RNTI check tells the UE.

<div class="guided-fold-start" data-label="The candidate-position formula and a numerical example" data-tone="derivation"></div>
...
### 12.4. Why UE-specific positions rotate every subframe {#sec-12-4}
...
### 12.5. False-positive probability {#sec-12-5}

The $2^{-16}$ CRC check is what makes blind decoding safe: a wrong candidate almost always fails it. A naive multiplication of $2^{-16}$ by the raw candidate count overstates the real false-DCI rate, though, because plausibility checks and conditional decoding cut the effective trial count well below the paper number.

<div class="guided-fold-start" data-label="Walking the false-positive arithmetic" data-tone="derivation"></div>
...
Several effects push the real rate much lower:
...
### 13.1. What the scheduler needs, and where it gets it {#sec-13-1}
...
### 13.2. DCI formats — the downlink command menu {#sec-13-2}
...
### 13.3. UCI — the uplink report channel {#sec-13-3}
...
### 13.4. HARQ — DL asynchronous vs UL synchronous {#sec-13-4}
...
### 13.6. Complete transactions with retransmission {#sec-13-6}
...
### 13.7. BSR — the volume report inside a MAC PDU {#sec-13-7}
...
### 14.3. RLC — segmentation and ARQ {#sec-14-3}
...
### 14.4. PDCP — ciphering, integrity, header compression {#sec-14-4}
...
<div class="guided-fold-start" data-label="Historical context: RoHC before LTE" data-tone="derivation"></div>
...
<div class="guided-fold-start" data-label="Historical context: PDCP before LTE" data-tone="derivation"></div>
...
### 14.5. RRC — the radio-configuration protocol {#sec-14-5}
...
### 15.1. The three states {#sec-15-1}
...
### 15.2. The typical combined states {#sec-15-2}
...
The ECM-CONNECTED with RRC_IDLE combination is impossible: without an RRC connection, there is no way for NAS messages to reach the eNodeB and then the MME, so ECM must be IDLE too. ECM and RRC transition together on the UE side.

### 15.3. What idle actually is — and what triggers exit from it {#sec-15-3}
...
The two exits differ in who initiates. From the RRC side both look similar (PRACH followed by RRC connection setup); from the UE side they are distinct scenarios.

### 15.4. DRX in idle vs connected {#sec-15-4}
...
### 15.5. Tracking area updates {#sec-15-5}
...
### 15.6. GUTI reallocation {#sec-15-6}
...
### 16.1. The three axes: scope, lifetime, secrecy {#sec-16-1}
...
### 16.2. IMSI — the permanent subscriber identity (network layer, global scope) {#sec-16-2}
...
### 16.3. GUTI — the temporary routable identity (network layer, MME-scoped) {#sec-16-3}
...
GUTI = |  MCC  |  MNC  | MME Group ID |  MME Code  |    M-TMSI    |
         3       2/3       16 bits        8 bits       32 bits
        digs    digs

       |------ PLMN ID ------|---- MME identity in PLMN ----|-- in MME --|
       |------------------- GUMMEI -------------------------|-- M-TMSI --|
~~~
...
<div class="guided-fold-start" data-label="Historical context: from TMSI and P-TMSI to GUTI" data-tone="derivation"></div>
...
### 16.4. The network vs radio identifier split {#sec-16-4}
...
The eNodeB is the *translator* between the two. On the radio side it addresses UEs by C-RNTI. On the S1-MME side it addresses UEs by an **eNB-UE-S1AP-ID / MME-UE-S1AP-ID** pair, and forwards NAS messages that carry a GUTI. It maintains an internal table binding, for each UE:

- `C-RNTI` (on the current cell)
- `eNB-UE-S1AP-ID` (this eNodeB's handle for the UE on S1-MME)
- `MME-UE-S1AP-ID` (the MME's handle for the UE on the same S1-MME association)
- `GUTI` (opaque to the eNodeB — carried inside the NAS payload it forwards)

The MME sees only the network-layer identifiers; the physical layer sees only the radio identifiers.

**Consequence for the two threat models.** A radio-layer attacker (fake eNodeB, passive sniffer of PDCCH) can observe C-RNTIs and correlate a UE across seconds within one cell — but every handover reshuffles them. A network-layer attacker with access to SGi or the operator's core sees GUTIs and IMSIs, which are much longer-lived and enable much stronger tracking. LTE's identifier design distinguishes the two.
...
### 16.6. Cell and area identifiers {#sec-16-6}
...
### 16.7. Summary — which identifier answers which question {#sec-16-7}
...
<div class="pull-out" markdown="1">
**The rule underneath the mess.** Each layer wants an identifier of just the right size and just the right scope, and permanent identifiers are hidden as much as possible while temporary ones are used publicly.
</div>
...
### 17.2. Bearer vs DRB — the difference in one sentence {#sec-17-2}
...
### 17.4. QCI and what "best effort" actually means {#sec-17-4}
...
### 17.5. ARP — the preemption sledgehammer {#sec-17-5}
...
### 17.6. The internet + IMS parallel bearer architecture {#sec-17-6}
...
### 18.2. The EPS-AKA exchange {#sec-18-2}
...
### 18.3. The key derivation function {#sec-18-3}
...
<div class="guided-fold-start" data-label="How LTE encodes the KDF input string" data-tone="derivation"></div>
...
### 18.4. The key derivation tree {#sec-18-4}
...
Specifically, the ciphered payload of a PDCP packet is

$$\text{cipher}(K_{\text{UPenc}}, \text{COUNT}, \text{DIRECTION}, \text{BEARER}, \text{LENGTH}) \oplus \text{plaintext}.$$
...
### 18.5. Purpose of each key {#sec-18-5}
...
### 18.6. Two security modes: NAS and AS {#sec-18-6}
...
### 18.7. Why user-plane has ciphering but no integrity {#sec-18-7}
...
### 18.8. Handover key management: forward and backward security {#sec-18-8}
...
### 19.1. The seven parties {#sec-19-1}
...
### 19.2. The message flow {#sec-19-2}
...
### 19.3. The piggybacked PDN Connectivity Request {#sec-19-3}
...
### 19.4. Where the security handshake sits {#sec-19-4}
...
### 19.5. What state the UE is in at each point {#sec-19-5}
...
### 20.1. The full picture {#sec-20-1}
...
### 20.2. S1: the eNodeB-to-core split {#sec-20-2}
...
### 20.4. S5 and S8: SGW to PGW {#sec-20-4}
...
### 20.7. X2: eNodeB to eNodeB {#sec-20-7}
...
### 20.8. GTP: the tunneling substrate {#sec-20-8}
...
### 21.1. Cell reselection (RRC_IDLE mode) {#sec-21-1}
...
### 21.3. X2 handover — the fast path {#sec-21-3}
...
### 21.4. S1 handover — the fallback path {#sec-21-4}
...
### 21.5. Measurement gaps {#sec-21-5}
...
### 22.2. Identities in IMS: IMPI, IMPU, and why more than one {#sec-22-2}
...
### 22.3. The three CSCFs — what each is *for* {#sec-22-3}
...
### 22.4. SIP: methods and what they do {#sec-22-4}
...
### 22.6. PCRF and the policy interface {#sec-22-6}
...
### 22.7. The complete VoLTE call setup {#sec-22-7}
...
### 22.8. Codec negotiation {#sec-22-8}
...
### 22.9. Emergency calls: a special mode {#sec-22-9}
...
### 23.2. SMS: legacy on everything {#sec-23-2}
...
### 23.3. Voicemail: 1985 architecture in 2025 {#sec-23-3}
...
### 23.4. Emergency calls, complete flow {#sec-23-4}
...
### 24.2. How the visited network learns to serve you {#sec-24-2}
...
### 24.3. Home routing vs local breakout {#sec-24-3}
...
### 24.5. The "data roaming" switch {#sec-24-5}
...
### 25.2. Timing advance as a distance signal {#sec-25-2}
...
### 25.5. SMS-over-SS7 and the broken 2FA weakness {#sec-25-5}
...
### 26.2. Fading models — Rayleigh, Rice, Nakagami {#sec-26-2}
...
- **Rayleigh fading.** No line-of-sight component. $|H|$ follows a Rayleigh distribution:

  $$f(r) = \frac{r}{\sigma^2} e^{-r^2/(2\sigma^2)}.$$

  Typical of dense urban with many scatterers, no direct path. RMS gain is $\sigma\sqrt{\pi/2}$.
...
### 26.4. SFN math — deriving paging occasions {#sec-26-4}

<div class="guided-fold-start" data-label="Derive the paging frame and paging occasion from the IMSI" data-tone="derivation"></div>
...
### 26.5. The 15 MHz FFT choice — 1536 or 2048 with padding {#sec-26-5}
...
<div class="pull-out" markdown="1">
**General principle.** Zero-padding in the frequency domain corresponds to interpolation in the time domain. A 2048-point IFFT of a signal with only 900 nonzero subcarriers produces a time-domain signal that is a $\sim 4/3$ upsampled version of what a native 1536-point IFFT would produce. Both contain the same information; the padded version just has more samples per second.
</div>

### 26.6. Airplane mode {#sec-26-6}
...
### 26.7. Find My Phone — the mechanism {#sec-26-7}
